...
export function OrganizationPage() {
  ...
  useEffect(() => {
    fetchStatus();
    const interval = setInterval(fetchStatus, 10000); // 10s Live Ping
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <p className="text-slate-500">Autonomous Agent Infrastructure</p>
            <div className="flex items-center gap-1.5 px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-bold border border-indigo-100">
              <RefreshCw size={10} className="animate-spin" />
              LIVE PING: {lastPing || 'Connecting...'}
            </div>
          </div>
        </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-slate-400">
                      <span>SYNC</span>
                      <span className="text-indigo-600 uppercase">{status?.system?.uptime || 'Active'}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-6">
                <div className="flex items-center gap-5 mb-4">
                  <div className="relative">
                    ...
                    <div className={`absolute -top-2 -right-2 bg-white p-1.5 rounded-full shadow-md ${status?.system?.uptime === 'Active' ? 'text-blue-600' : 'text-slate-400'}`}>
                      <Cpu size={14} />
                    </div>
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-slate-800 text-lg leading-tight">{status?.system?.model.split('/').pop() || 'Claude 3.5 Sonnet'}</h4>
                    <p className="text-sm text-slate-500 font-medium mt-1">v{status?.system?.version || '7.2'}</p>
                  </div>
                </div>
                ...
                {expandedNodes.includes('assetlink') && (
                  <div className="mt-8 space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { label: 'Context', value: status?.system?.tokens?.context || '---', color: 'text-indigo-600' },
                        { label: 'Total Tokens', value: (status?.system?.tokens?.total / 1000).toFixed(1) + 'k' || '---', color: 'text-blue-600' },
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['browser', 'exec', 'write', 'read'].map(tool => (
                          <div key={tool} className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl flex items-center gap-3">
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 flex justify-between items-center">
                   <div className="flex items-center gap-2">
                      <div className={`w-2.5 h-2.5 rounded-full ${status?.system?.uptime === 'Active' ? 'bg-blue-500 animate-pulse' : 'bg-slate-300'}`}></div>
                      <span className="text-[11px] font-bold text-slate-400 uppercase">Synchronized</span>
                   </div>
                   <div className="flex gap-2">
                      <button className="p-2 bg-slate-50 hover:bg-blue-600 hover:text-white rounded-xl text-slate-400 transition-all"><Globe size={16} /></button>
                      <button className="p-2 bg-blue-600 text-white rounded-xl shadow-lg transition-all"><Settings size={16} /></button>
                   </div>
                </div>
              </div>
